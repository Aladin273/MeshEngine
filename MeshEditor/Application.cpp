...
View* Application::createView(const std::string& title, uint32_t width, uint32_t height)
{
    ...
    m_views.back()->addOperator(KeyCode::E, KeyCode::Escape, std::make_unique<EditMeshOperator>());
    
    m_views.back()->addOperator(KeyCode::W, KeyCode::Escape, std::make_unique<EditVertexOperator>());

    m_views.back()->addOperator(KeyCode::T, KeyCode::Escape, std::make_unique<TransformNodeOperator>());
